#!/usr/bin/env python3
"""Rewrite min_kv_buffer_mb / MIN_KV_BUFFER_MB in e2e tests from CI logs.

Mines scheduled/nightly logs for KV allocation lines (or explicit
``kv_buffer_mb``) and writes a single hardware-keyed threshold per test file::

    MIN_KV_BUFFER_MB = {"h200": 12000.0, "b200": 18000.0}

or a scalar when only one GPU family was observed.
...
def parse_job_log(text: str, *, job_name: str) -> List[Tuple[str, str, float]]:
    ...
    for tf in order:
        ...
        if not vals:
            ...
            if mb is not None:
                vals.append(mb)
        # One threshold per (test, gpu): use the largest observed KV buffer
        # (main server, not a tiny draft worker).
        if vals:
            out.append((tf, gpu, max(vals)))
    return out
...
def aggregate(
    ...
    """test_file -> gpu_family -> single floor (mean * factor)."""
